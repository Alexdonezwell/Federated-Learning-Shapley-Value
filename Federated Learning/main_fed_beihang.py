...
#####################################################################################################
    ...
    for iter in range(args.epochs):
        ...
        for idx in idxs_users:
            ...
            #update each client's w in the client_w_dict
            client_w_dict[idx] = w
    ...
    with open('dict_beihang.csv', 'w') as csv_file:  
    ...
    # sys.exit(0)
    # user_order_arry = np.array(users_order_lst, dtype=object)
    # index = 0
    # user_lst = list(range(0, 10))
    # init_lst = [0 for i in range(10)]
    # user_dict = dict(zip(user_lst,init_lst))
    # print(user_dict)
    # acc_test = 0
    # for iter in range(args.epochs):
    ...
    #     m = max(int(args.frac * args.num_users), 1)
    #     #picking clients in the order we want
    #     idxs_users = user_order_arry[index]
    #     index +=1
    #     # idxs_users = np.random.choice(range(args.num_users), m, replace=False)
    #     print('======',idxs_users,type(idxs_users))
    #     for idx in idxs_users:
    #         # print(type(idx),dict_users[idx])
    #         #used for update the real model
    #         local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
    #         w, loss = local.train(net=copy.deepcopy(net_glob).to(args.device))
    #         w_locals.append(copy.deepcopy(w))
    #         loss_locals.append(copy.deepcopy(loss))

    #         #used to update the marginal contribution of each element in record model
    #         local_record = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx])
    #         w_r, loss_r = local.train(net=copy.deepcopy(net_glob_record).to(args.device))
    ...
    #         user_dict[idxs_users[i]] += abs(acc_test -prev_acc_test)/len(idxs_users)
